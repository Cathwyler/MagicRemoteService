	...
	public partial class Setting : System.Windows.Forms.Form {
		...
		public static void AppExtract(
			...
			System.IO.File.WriteAllText(".\\TV\\MagicRemoteService\\main.js", MagicRemoteService.Properties.Resources.main
				...
				.Replace("const sInputId = \"HDMI_1\";", "const sInputId = \"" + wcdiInput.strId + "\";")
				.Replace("const sInputName = \"HDMI 1\";", "const sInputName = \"" + wcdiInput.strName + "\";")
				.Replace("const sInputSource = \"ext://hdmi:1\";", "const sInputSource = \"" + wcdiInput.strSource + "\";")
				.Replace("const sInputAppId = \"com.webos.app.hdmi1\";", "const sInputAppId = \"" + wcdiInput.strAppId + "\";")
			...
			System.IO.File.WriteAllText(".\\TV\\MagicRemoteService\\appinfo.json", MagicRemoteService.Properties.Resources.appinfo
				.Replace("\"id\": \"com.cathwyler.magicremoteservice\"", "\"id\": \"com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort + "\"")
			...
			System.IO.File.WriteAllText(".\\TV\\Send\\package.json", MagicRemoteService.Properties.Resources.package
				.Replace("\"name\": \"com.cathwyler.magicremoteservice.send\"", "\"name\": \"com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort + ".send\"")
			);
			...
			System.IO.File.WriteAllText(".\\TV\\Send\\services.json", MagicRemoteService.Properties.Resources.services
				.Replace("\"id\": \"com.cathwyler.magicremoteservice.send\"", "\"id\": \"com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort + ".send\"")
			);
		}
			...
			if(this.cmbboxTV.SelectedItem == null) {
				...
				if(!await System.Threading.Tasks.Task.Run<bool>(delegate () {
					try {
						...
						MagicRemoteService.WebOSCLI.Install(wcdDevice.strName, ".\\TV\\com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort + "_" + strVersion + "_all.ipk");
						MagicRemoteService.WebOSCLI.Launch(wcdDevice.strName, "com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort);
						return true;
					} catch(System.Exception ex) {
		...
		private void Setting_FormClosing(object sender, System.Windows.Forms.FormClosingEventArgs e) {
			...
			if(!e.Cancel && (
				this.wcdiInput?.strId != ((MagicRemoteService.WebOSCLIDeviceInput)this.cmbboxInput.SelectedItem)?.strId
				||
					...
					if(!await System.Threading.Tasks.Task.Run<bool>(delegate () {
						try {
							...
							if(!taDialog.Advanced) {
								MagicRemoteService.WebOSCLI.NovacomGetKey(taDialog.Device.strName, taDialog.Device.wcddDeviceDetail.strPassphrase);
							}
							return true;
						} catch(System.Exception ex) {
		...
		private async void TVModify_Click(object sender, System.EventArgs e) {
			if(this.cmbboxTV.SelectedItem == null) {
				...
				MagicRemoteService.WebOSCLIDevice wcdDevice = (MagicRemoteService.WebOSCLIDevice)this.cmbboxTV.SelectedItem;
				MagicRemoteService.TVAdder taDialog = new MagicRemoteService.TVAdder(wcdDevice);
				switch(taDialog.ShowDialog()) {
					case System.Windows.Forms.DialogResult.OK:
						this.Enabled = false;
						string strError = null;
						...
						if(!await System.Threading.Tasks.Task.Run<bool>(delegate () {
							try {
								MagicRemoteService.WebOSCLI.SetupDeviceModify(wcdDevice.strName, taDialog.Device, taDialog.Password);
								if(!taDialog.Advanced) {
									MagicRemoteService.WebOSCLI.NovacomGetKey(taDialog.Device.strName, taDialog.Device.wcddDeviceDetail.strPassphrase);
								}
								return true;
		...
		private async void TVRemove_Click(object sender, System.EventArgs e) {
			if(this.cmbboxTV.SelectedItem == null) {
				...
				if(!await System.Threading.Tasks.Task.Run<bool>(delegate () {
					try {
						MagicRemoteService.WebOSCLI.SetupDeviceRemove(wcdDevice.strName);
						return true;
					} catch(System.Exception ex) {
		...
		private void Inspect_Click(object sender, System.EventArgs e) {
			async void Inspect() {
				...
				if(!await System.Threading.Tasks.Task.Run<bool>(delegate () {
					try {
						MagicRemoteService.WebOSCLI.Inspect(wcdDevice.strName, "com.cathwyler.magicremoteservice." + wcdiInput.strAppIdShort);
						return true;
					} catch(System.Exception ex) {
						strError = MagicRemoteService.Properties.Resources.SettingTVInstallErrorTitle;
						strErrorInfo = ex.Message;
